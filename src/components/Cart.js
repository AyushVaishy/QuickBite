    ...
    <div className="flex flex-col md:flex-row justify-center items-start min-h-screen bg-gray-100 dark:bg-gray-900 pt-24 pb-10 px-2 md:px-10">
      ...
      <div className="w-full md:w-1/3 max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 sticky top-24">
        ...
        <div className="flex items-center mb-4">
          ...
          <div>
            <div className="font-bold text-lg text-gray-800 dark:text-gray-100">{cartItems[0]?.card?.info?.restaurantName || "Pizza Hut"}</div>
          ...
          {cartItems.map((item, idx) => (
            <div key={item.card?.info?.id || idx} className="flex items-center justify-between mb-2">
              <div className="flex items-center">
                <span className="font-semibold text-gray-800 dark:text-gray-100">{item.card?.info?.name}</span>
                <span className="ml-2 text-gray-500 dark:text-gray-400 text-sm">x{item.quantity || 1}</span>
              </div>
